...
import SalesOrderCreateForm from "./SalesOrderCreateForm.jsx";
...
export default function SalesOrderPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return [...orders]
      ...
      .filter((order) => {
        ...
        return [order.soNumber, order.customer?.displayName, order.marketplace?.customer, order.orderTypeLabel].some((value) => String(value ?? "").toLowerCase().includes(keyword));
      })
      .sort((a, b) => String(b.soNumber).localeCompare(String(a.soNumber), undefined, { numeric: true }));
  }, [orders, search, statusFilter]);

  function refreshOrders() { setOrders(salesOrderStore.getSalesOrders()); }
  function handleCreated(order) { refreshOrders(); setShowNewOrder(false); setSelectedOrder(order); }
  function handleSaved(order) { refreshOrders(); setEditingOrder(null); setSelectedOrder(order); }
  function handleCancelled(order) { refreshOrders(); setSelectedOrder(order); }

  if (showNewOrder) return <SalesOrderCreateForm onCancel={() => setShowNewOrder(false)} onCreated={handleCreated} />;
  if (editingOrder) return <SalesOrderCreateForm initialOrder={editingOrder} onCancel={() => setEditingOrder(null)} onSaved={handleSaved} />;
  ...
  return (
    <div className="sales-order-page">
      ...
      <section className="ui-card sales-order-list-card">
        <div className="sales-order-list-toolbar">
          ...
          <select className="ui-input" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
            <option value="ALL">All Status</option><option value="NEW ORDER">New Order</option><option value="READY PRODUCTION">Ready WO</option><option value="IN PRODUCTION">In Progress</option><option value="PACKING">Packing</option><option value="RTS">RTS</option><option value="COMPLETED">Completed</option><option value="INACTIVE">Inactive</option>
          </select>
        </div>
      ...
      {selectedOrder && <SalesOrderDetailDrawer order={selectedOrder} onClose={() => setSelectedOrder(null)} onEdit={(order) => { setSelectedOrder(null); setEditingOrder(order); }} onCancelled={handleCancelled} />}
    </div>
  );
}
