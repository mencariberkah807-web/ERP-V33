...
import { createSalesOrderFromForm } from "./services/salesOrderService.js";
...
function createItem() {
  return { id: `${Date.now()}-${Math.random()}`, productId: "", quantity: 1, unitPrice: "", discount: "", notes: "", attachment: null };
}
...
export default function SalesOrderCreateFormV3({ onCancel, onCreated }) {
  ...
  const grandTotal = useMemo(() => items.reduce((total, item) => total + itemTotal(item), 0), [items]);
  ...
  function validate() {
    ...
    if (Number(amountPaid) < 0) return "Amount Paid tidak boleh negatif.";
    if (orderType === "DIRECT" && Number(amountPaid) > grandTotal) return "Amount Paid tidak boleh melebihi Grand Total.";
    return "";
  }
  ...
  function handleSubmit(event) {
    ...
    setError("");
    try {
      const order = createSalesOrderFromForm({ orderType, orderDate, deadline, priority, customer: selectedCustomer, marketplace, marketplaceCustomer, trackingNumber, items, amountPaid, paymentMethod, paymentReference });
      onCreated(order);
    } catch (saveError) {
  ...
  return (
    <form className="sales-order-create-v3" onSubmit={handleSubmit}>
      <header className="sales-order-header"><div><h1>New Sales Order</h1><p>Create a new sales order</p></div></header>
      ...
      <section className="ui-card">
        ...
        <div className="sales-order-card-body sales-order-radio-row">
          <label className="sales-order-radio"><input type="radio" name="orderType" value="DIRECT" checked={orderType === "DIRECT"} onChange={() => setOrderType("DIRECT")} />Direct Order</label>
          <label className="sales-order-radio"><input type="radio" name="orderType" value="MARKETPLACE" checked={orderType === "MARKETPLACE"} onChange={() => setOrderType("MARKETPLACE")} />Marketplace</label>
        </div>
      </section>
      ...
      <div className="sales-order-two-column">
        <section className="ui-card">
          ...
          <div className="sales-order-card-body sales-order-grid-2">
            <Field label="SO Number"><input className="ui-input" value="Generated by system" readOnly /></Field>
            ...
            <div className="sales-order-item" key={item.id}>
              <div className="sales-order-item-main">
                <div className="sales-order-grid-2">
                  <Field label="Product"><select className="ui-input" value={item.productId} onChange={(event) => handleProductChange(item.id, event.target.value)}><option value="">Select Product...</option>{products.map((product) => <option key={product.productId} value={product.productId}>{product.productName} {product.sku ? `(${product.sku})` : ""}</option>)}</select></Field>
                  <div className="sales-order-grid-3">
                    <Field label="Quantity"><input className="ui-input" type="number" min="1" value={item.quantity} onChange={(event) => updateItem(item.id, "quantity", event.target.value)} /></Field>
                    <Field label="Unit Price"><input className="ui-input" type="number" min="0" value={item.unitPrice} onChange={(event) => updateItem(item.id, "unitPrice", event.target.value)} /></Field>
                    <Field label="Discount"><input className="ui-input" type="number" min="0" value={item.discount} onChange={(event) => updateItem(item.id, "discount", event.target.value)} /></Field>
                  </div>
                </div>
                <div className="sales-order-item-extra">
                  <Field label="Production Notes / Special Request"><textarea className="sales-order-textarea" rows="2" value={item.notes} onChange={(event) => updateItem(item.id, "notes", event.target.value)} /></Field>
                  <Field label="Artwork"><input className="sales-order-file" type="file" onChange={(event) => updateItem(item.id, "attachment", event.target.files?.[0] || null)} /></Field>
                </div>
              </div>
              <div className="sales-order-item-total"><Field label="Item Total"><input className="ui-input" value={formatIDR(itemTotal(item))} readOnly /></Field>{items.length > 1 && <button type="button" className="sales-order-remove-button" onClick={() => setItems((current) => current.filter((entry) => entry.id !== item.id))}>Remove Item</button>}</div>
            </div>
          ))}
        </div>
      </section>

      {orderType === "DIRECT" && <section className="ui-card"><div className="sales-order-card-header">Payment</div><div className="sales-order-card-body"><div className="sales-order-grid-3"><Field label="Grand Total"><input className="ui-input" value={formatIDR(grandTotal)} readOnly /></Field><Field label="Amount Paid"><input className="ui-input" type="number" min="0" value={amountPaid} onChange={(event) => setAmountPaid(event.target.value)} /></Field><Field label="Balance"><input className="ui-input" value={formatIDR(balance)} readOnly /></Field></div><div className="sales-order-payment-method sales-order-margin-top"><strong>Payment Method</strong>{["Cash", "Transfer", "QRIS"].map((value) => <label className="sales-order-payment-radio" key={value}><input type="radio" name="paymentMethod" value={value} checked={paymentMethod === value} onChange={() => setPaymentMethod(value)} />{value}</label>)}<input className="ui-input sales-order-payment-reference" value={paymentReference} onChange={(event) => setPaymentReference(event.target.value)} placeholder="Payment reference" /></div></div></section>}

      <footer className="sales-order-footer"><button type="button" className="sales-order-secondary-button" onClick={onCancel}>Cancel</button><button type="submit" className="ui-button-primary" disabled={saving}>{saving ? "Saving..." : orderType === "MARKETPLACE" ? "Create WO" : "Create Order"}</button></footer>
    </form>
  );
}
