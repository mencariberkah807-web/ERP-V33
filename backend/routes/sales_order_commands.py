...
def _idempotency(db, key, endpoint, payload):
    ...
    if record.endpoint != endpoint or stored.get("_requestHash") != _fingerprint(payload):
...
def _store(db, key, endpoint, payload, response):
    stored = dict(response); stored["_requestHash"] = _fingerprint(payload)
    db.add(IdempotencyRecord(idempotency_key=key.strip(), endpoint=endpoint, response_data=stored))
...
def list_sales_orders(
    ...
    sort: str = Query(default="sales_order_id"),
    orderType: str | None = Query(default=None),
    ...
    paymentStatus: str | None = Query(default=None),
):
    allowed_sorts = {
    ...
    with Session(engine) as db:
        ...
        if customerId:
            normalized = customerId.strip()
            customer = db.scalar(select(Customer).where(Customer.customer_code == normalized))
        ...
        if paymentStatus:
            normalized_payment = paymentStatus.strip().upper().replace(" ", "_")
            paid = func.coalesce(func.sum(Payment.amount), 0)
            if normalized_payment == "PAID": conditions.append(SalesOrder.sales_order_id.in_(select(Payment.sales_order_id).group_by(Payment.sales_order_id).having(paid > 0)))
            elif normalized_payment == "UNPAID": conditions.append(~SalesOrder.sales_order_id.in_(select(Payment.sales_order_id).group_by(Payment.sales_order_id).having(paid > 0)))
            else: raise HTTPException(status_code=422, detail="Unsupported paymentStatus; use PAID or UNPAID")
        base = select(SalesOrder).where(*conditions)
        ...
        rows = db.scalars(base.order_by(ordering, SalesOrder.sales_order_id.asc()).offset((page - 1) * pageSize).limit(pageSize)).all()
        result=[]
        for record in rows:
            ...
            result.append({
                ...
                "customerId": customer.customer_code if customer and customer.customer_code else str(record.customer_id),
                "orderDate": record.order_date.isoformat() if record.order_date else None,
                "deadline": record.deadline.isoformat() if record.deadline else None,
                "priority": record.priority,
                "marketplace": record.marketplace,
                "marketplaceCustomer": record.marketplace_customer,
...
def update_sales_order(so_number, payload: SalesOrderUpdateInput, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")):
    ...
    with Session(engine) as db:
        ...
        response=_success({"soNumber":record.order_number,"status":record.status,"priority":record.priority,"orderDate":record.order_date.isoformat() if record.order_date else None,"deadline":record.deadline.isoformat() if record.deadline else None})
        _store(db,idempotency_key,endpoint,request_data,response); db.commit(); return response
