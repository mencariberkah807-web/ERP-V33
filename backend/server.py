...
def _error_response(status_code: int, message: str, details=None):
    return JSONResponse(status_code=status_code, content={"error": {"code": ERROR_CODES.get(status_code, "API_ERROR"), "message": message, "details": details or {}}})
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    if isinstance(detail, dict):
        ...
        code = detail.get("code") or ERROR_CODES.get(exc.status_code, "API_ERROR")
...
async def request_validation_exception_handler(request: Request, exc: RequestValidationError):
    return _error_response(422, "Request validation failed", {"errors": exc.errors()})
